...
ig.Game = ig.Class.extend({
	...
	drawEntities: function() {
		for( var i = 0; i < this.entities.length; i++ ) {
			if(this.entities[i].name == 'point' && this.entities[i].isDrawn == false){
				this.entities[i].draw();
				//this.entities[i].isDrawn = true;
				//console.log('draw once',i);
			}
			else if(this.entities[i].name != 'point'){
				this.entities[i].draw();
			}
		}
	},
	...
	checkEntities: function() {
		...
		for( var e = 0; e < this.entities.length; e++ ) {
			if(this.entities[e].name == 'point'){
				
			}
			else if(this.entities[e].name != 'point'){
